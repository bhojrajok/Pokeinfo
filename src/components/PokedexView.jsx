  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden w-full">
      ...
      <header className={`${cls.surfaceMuted} border-b ${cls.border} p-4 shrink-0 transition-colors duration-200`}>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-3 w-full max-w-4xl">
            <div className="relative w-full sm:w-80">
              <input 
                ...
                {...searchInputProps}
                className={`w-full ${cls.surface} border ${cls.border} rounded-xl pl-4 pr-10 py-2 text-xs font-bold focus:outline-none focus:border-blue-500 text-inherit transition-all placeholder-slate-400`}
              />
            ...
            <select
              {...genSelectProps}
              className={`w-full sm:w-44 ${cls.surface} border ${cls.border} rounded-xl px-3 py-2 text-xs font-bold focus:outline-none focus:border-blue-500 text-inherit cursor-pointer`}
            >
        ...
        <div className="max-w-[1800px] mx-auto mt-3 grid grid-cols-4 sm:grid-cols-7 md:grid-cols-10 gap-1 border-t border-slate-500/10 pt-2.5">
          {POKEMON_TYPES.map((type) => (
            <button
              key={type.name}
              onClick={() => typeFilterProps.onChange(type.name)}
              className={`flex items-center justify-center gap-1.5 py-1 px-2 rounded-lg text-[10px] font-black uppercase border transition-all truncate ${
      ...
      <div className="flex-1 flex flex-col lg:flex-row max-w-[1800px] w-full mx-auto p-4 gap-4 overflow-hidden">
        <main className={`${cls.surface} border ${cls.border} rounded-2xl p-4 overflow-y-auto custom-scrollbar flex-1 flex flex-col justify-between transition-colors duration-200`}>
          {loading ? (
            ...
            <div className="w-full">
              <div className="flex items-center justify-between mb-4 px-1">
                ...
                <div className={`text-[9px] font-mono font-bold uppercase ${cls.textMuted} bg-slate-500/5 px-2.5 py-1 rounded-md border ${cls.border}`}>
                  FILTER: {typeFilterProps.value}
                </div>
              </div>

              {/* Clicking here loads data silently now */}
              <div className="grid grid-cols-[repeat(auto-fill,minmax(140px,1fr))] gap-3 w-full">
                {paginatedPokemon.map((poke, index) => (
                  ...
                  >
                    ...
                    <div className={`w-16 h-16 ${cls.surface} rounded-xl p-1.5 flex items-center justify-center border ${cls.border} group-hover:scale-105 transition-transform`}>
                      <img src={`https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/other/official-artwork/${poke.id}.png`} alt={poke.name} className="w-full h-full object-contain filter drop-shadow z-10" />
                    </div>
              ...
              {processedPokemon.length > visibleCount && (
                <div className="mt-6 text-center">
                  <button onClick={() => setVisibleCount(prev => prev + 100)} className="bg-blue-600 hover:bg-blue-500 text-white font-black text-[10px] tracking-widest px-6 py-3 rounded-xl uppercase transition-all shadow-sm">Load Next 100 Entries</button>
                </div>
              )}
            </div>
          )}
        ...
        <section className={`w-full lg:w-[380px] ${cls.surface} border ${cls.border} rounded-2xl p-4 flex flex-col overflow-y-auto custom-scrollbar h-auto lg:h-full shrink-0 shadow-lg transition-colors duration-200`}>
          {detailLoading ? (
            <div className="flex-1 flex flex-col items-center justify-center py-20"><div className="w-5 h-5 border-2 border-blue-500 border-t-transparent rounded-full animate-spin"></div></div>
          ) : selectedPokemon ? (
            <div className="space-y-4 flex flex-col h-full justify-between">
              <div>
                <div className="text-center relative mt-8">
                  ...
                  {/* Interactive Playcard Image Frame (Only plays sound when clicked here) */}
                  <div 
                ...
                <div className="space-y-1.5 mt-4">
                  ...
                  <div className={`${cls.surfaceMuted} p-2 rounded-xl border ${cls.border} flex items-center justify-around gap-1`}>
                    {evolutionChain.map((node, index) => (
                      <React.Fragment key={node.id}>
                        {index > 0 && <span className={`${cls.textMuted} text-xs font-bold`}>➔</span>}
                        <button 
                        ...
                        >
                          <div className={`w-10 h-10 ${cls.surface} rounded border ${cls.border} flex items-center justify-center`}>
                            <img src={`https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/${node.id}.png`} alt={node.name} className="w-full h-full object-contain" />
                          </div>
                        </button>
                      </React.Fragment>
                    ))}
              ...
              <div className="space-y-2 mt-2">
                ...
                <div className={`${cls.surfaceMuted} p-3 rounded-xl border ${cls.border} space-y-1.5`}>
                  {selectedPokemon.stats.map((s, idx) => (
                    <div key={idx} className="space-y-0.5">
                      <div className="flex justify-between text-[9px] font-mono font-bold"><span className={`capitalize ${cls.text}`}>{s.stat.name.replace('-', ' ')}</span><span className={cls.text}>{s.base_stat}</span></div>
                      <div className={`w-full h-1 ${cls.surface} rounded-full overflow-hidden border ${cls.border}`}><div className="h-full bg-blue-500 transition-all rounded-full" style={{ width: `${Math.min((s.base_stat / 255) * 100, 100)}%`, backgroundColor: themeColor }}></div></div>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className={`flex-1 flex flex-col items-center justify-center text-center p-6 border-2 border-dashed ${cls.border} rounded-xl ${cls.surfaceMuted} my-auto`}><span className="text-2xl mb-1 opacity-20">📊</span><h3 className={`text-[10px] font-black tracking-widest ${cls.textMuted} uppercase`}>Select Entry</h3></div>
          )}
        </section>
      </div>
    </div>
